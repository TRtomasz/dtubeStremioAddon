...
var manifest = { 
    ...
    logo: "https://about.d.tube/img/kit/Logo_White_D.png",
...
function loadPaginatedVideos(users,counter,callback,page,maxAmount,returnArr)
{
    ...
    request(req, function(e,r,b){
        if(!e && r.statusCode == 200)
        {
            ...
            console.log(videos.length +'a');
                    if(videos.length != 0){
                        for(i = 0, j = videos.length; i < j; i++)
                        {
                            var video = 
                            {
                                id: videos[i].meta.video.content.video480hash ? linkByHash(videos[i].meta.video.content.video480hash) : linkByHash(videos[i].meta.video.content.videohash),
...
var addon = new Stremio.Server({
    ...
    "meta.find": function(args, callback) {
        ...
        if(args.skip)
        {
            page = Math.ceil(args.skip / 10) + 1
        }
        else
            page = 1
        console.log("Page ", page);
        if(page < 10)
         loadPaginatedUsersWith([],page,60,callback);
    },
